...
# coding: utf-8
"""
train_s3dconv1d.py — Full training pipeline for S3D-Conv1D-Flat on WLASL + Samuel dataset.
...
Steps:
  ...
  5. Build, compile, and train the S3DConv1D-Flat model
...
def build_s3d_conv1d_flat(input_shape=INPUT_SHAPE, num_classes=NUM_CLASSES):
    T, H, W, C = input_shape
    H_out  = H // 4
    W_out  = W // 4
    ...
    # Temporal Conv1D
    x = Conv1D(48, 3, padding='same', activation='relu', name="temporal_conv1d")(x)
...
model = build_s3d_conv1d_flat(input_shape=INPUT_SHAPE, num_classes=NUM_CLASSES)
model.summary()
